...
public class SlimeyShader extends AbstractShader {
	...
	public SlimeyShader(Lightsource lightsource){
		super(lightsource);
...
//		float y = texel_v;			
		 ...
		 * */		
		float noise = (float)NoiseGenerator.improvedPerlinNoise(x*0.5, y*0.5, z*0.5); 
...
//		float gray = (float)Math.sin(x*10*Math.abs(noise));
//		float gray = (float)Math.sin((x+y)*noise*10+timeFactor);
		
		float gray = 
		(	
			 ( 1f*(float)Math.sin( 4f* x+timeFactor+noise*5) ) 
