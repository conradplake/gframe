...
public class FlowFieldShader extends AbstractShader {
	...
	float zInc = 0.005f; // for slicing the noise cube along the z-axis over time
	...
	public FlowFieldShader(Lightsource lightsource, int numberOfParticles) {
		...
		for(int i=0;i<numberOfParticles;i++){
			Particle p = new Particle(new Point3D((float)(Math.random()*textureWidth), (float)(Math.random()*textureHeight), 0)); 
//			Particle p = new Particle(new Point3D(textureWidth/2, textureHeight/2, 0));
			particles.add(p);
		}
		
		field = new Vector3D[textureWidth * textureHeight];
		for (int i=0;i<field.length;i++) {
			//field[i] = new Vector3D((float)(-0.5+Math.random()), (float)(-0.5+Math.random()), (float)(-0.5+Math.random()));
...
//				CirclesShader.drawCircle(texture, new Circle(x, y, 0), particle.getRgb());
				
				// trim speed; better idea: average velocity vector with current force vector
...
//				| ((blue & 0xFF) << 0);
		 ...
		 return super.shade(alpha, red, green, blue,
