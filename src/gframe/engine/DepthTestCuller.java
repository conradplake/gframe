...
public class DepthTestCuller {
	...
	/**
	 * The sets a zBuffer against which to perform depth tests.
	 * We hold just a reference to the buffer, so changes made to it elsewhere will directly effect the test outcome.
	 * */
	...
	/**
	 * Returns true if given face is considered occluded.
	 * If true, the face is added to an internal buffer (markedOccluded)
	 * which can be accessed later, e.g. during a final testing stage 
	 * */	
...
//		if(isOccluded(renderFace)){
	...
	public boolean isOccluded(RenderFace renderFace) {
		...
		float bbox_minz = renderFace.minZ(); // take closest z-Value (we are most conservative here)  		
...
//		System.out.println("bbox completely covered");
	...
	public boolean isOccludedFAST(RenderFace renderFace) {
		...
		// plus center?
//		if(zBuffer.getValue(bbox_minx+(bbox_maxx-bbox_minx)>>1, bbox_miny+(bbox_maxy-bbox_miny)>>1) > bbox_minz){
//			return false;
//		}
