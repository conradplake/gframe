...
public class TextureShader extends AbstractShader{
	...
	public TextureShader(Lightsource lightsource) {		
		this(lightsource, new File("textures/chessboard.jpg"), 256, 256); // load default test image
	}
...
//			int y_int = (int)Math.floor(y + 0.5f);
			...
			int c3 = x_int+1<textureWidth && y_int+1<textureHeight? texture.getPixel(x_int+1, y_int+1) : pixel;
			...
			int newAlpha = (int)( c0_alpha * c0_weight
					+ c1_alpha * c1_weight
					+ c2_alpha * c2_weight
					+ c3_alpha * c3_weight );
			
			int newRed = (int)( c0_red * c0_weight
					+ c1_red * c1_weight
					+ c2_red * c2_weight
					+ c3_red * c3_weight );
					
			int newGreen = (int)( c0_green * c0_weight
					+ c1_green * c1_weight
					+ c2_green * c2_weight
					+ c3_green * c3_weight );
			
			int newBlue = (int)( c0_blue * c0_weight
					+ c1_blue * c1_weight
					+ c2_blue * c2_weight
					+ c3_blue * c3_weight );
			
			
			pixel =	((newAlpha & 0xFF) << 24) |    
					((newRed & 0xFF) << 16) |
	                ((newGreen & 0xFF) << 8)  |
	                ((newBlue & 0xFF) << 0);						
	...
	public int shade(RenderFace renderFace, float world_x, float world_y, float world_z, float normal_x, float normal_y, float normal_z, float texel_u, float texel_v, int screen_x, int screen_y) {
		...
		return super.shade(alpha, red, green, blue, world_x, world_y, world_z, normal_x, normal_y, normal_z);
	}
