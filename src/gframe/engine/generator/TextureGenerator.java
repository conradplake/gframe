...
public class TextureGenerator {
	...
	/**
	 * Returns a new texture scaled down by half the width and height of the original texture. 
	 * Each new pixel thus represents 4 original pixels by averaging their values. 
	 * */
	...
	public static ImageRaster generateMengerSpongeTexture(ImageRaster raster, int pos_x, int pos_y, int width, int cubeColor, int gapColor) {
		
		if(width%3!=0){
			System.out.println("TextureGenerator.generateMengerSpongeTexture: Warning! Parameter width="+width+" is no multiple of 3!");
		}
		...
		for(int s=0;s<3;s++){
			for(int t=0;t<3;t++){
				...
				if(blocksize % 3 == 0  && c == cubeColor){
					generateMengerSpongeTexture(raster, pos_x + (s*blocksize), pos_y + (t*blocksize), blocksize, cubeColor, gapColor);
				}else{
	...
	public static ImageRaster generateMengerSpongeNormalMap(ImageRaster raster, int pos_x, int pos_y, int width) {
		...
		if(width%3!=0){
			System.out.println("TextureGenerator.generateMengerSpongeNormalMap: Warning! Parameter width="+width+" is no multiple of 3!");
		}
...
//									normal = Toolbox.getXrotMatrix(180).transform(normal);
	...
	/*
	 * see also: https://fgiesen.wordpress.com/2010/03/28/how-to-generate-cellular-textures/
	 * 
	 * 	
	 */
...
//		long distToDiagonalNeighbor = manhattanDistance? 2000 : 1414; // do euclidian else					
				
		
		for(int y=0;y<h;y++){
			for(int x=0;x<w;x++){			
		
//				long noise = (long)(100 * NoiseGenerator.improvedPerlinNoise((float)w/x, (float)h/y, 0));				
//				System.out.println("noise = "+noise);
				long noise = 0L;
				
				int nodeId = 1 + y*w+x; // graph node ids start counting at with 1, hence: 1 + ...
...
//						}
						...
						//if(x%tile_size==0 || y%tile_size==0 || x==tile_size-1 || y==tile_size-1){ // fugen weiß
...
//						}
	...
	public static ImageRaster generateTileTexture(int w, int h, int tilesize, int tileColor1, int tileColor2, int fugenColor) {
...
//		Color fugenColor = new Color(0, 0, 255);
		...
		for(int r=0;r<numberOfRows;r++){
			...
			for(int t=0;t<tilesPerRow;t++){
				...
				switch(select){
					case 0: col = Color.red.getRGB(); break;
					case 1: col = Color.green.getRGB(); break;
					case 2: col = Color.blue.getRGB(); break;
					case 3: col = Color.yellow.getRGB(); break;
					case 4: col = Color.cyan.getRGB(); break;
					case 5: col = Color.magenta.getRGB(); break;
					default: col = Color.black.getRGB(); break;
				}					
...
//						}						
	...
	public static ImageRaster generateWoodTexture(int width, int height, int alpha){
		...
		for(int x=-width/2;x<width/2;x++){			
			float fx = (dimension_x/(float)width)*x; // x-wert normalisieren			
			
			for(int y=-height/2;y<height/2;y++){								
				float fy = (dimension_y/(float)height)*y; // y-wert normalisieren
